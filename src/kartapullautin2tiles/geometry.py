...
class GeographicCoordinate:
    ...
    def to_mercator(self):
        return MercatorCoordinate(
            self.lon * _originshift_180, math.log(math.tan((90 + self.lat) * _pi_360)) / _pi_180 * _originshift_180
        )

    def __str__(self):
        return "<lon: " + str(self.lon) + ", lat: " + str(self.lat) + ", height: " + str(self.height) + ">"
...
class GeographicBB:
    ...
    def intersects(self, other):
        return (
            ...
            and self.max.lon >= other.min.lon
            and self.min.lat <= other.max.lat
            and self.max.lat >= other.min.lat
        )
    ...
    def intersection(self, other):
        ...
        if intersects:
            return GeographicBB(
                max(self.min.lon, other.min.lon),
                max(self.min.lat, other.min.lat),
                min(self.max.lon, other.max.lon),
                min(self.max.lat, other.max.lat),
            )
        elif self.max.lon > 180.0:
            return GeographicBB(-180, self.min.lat, self.max.lon - 360.0, self.max.lat).intersection(other)

    def center(self):
        return GeographicCoordinate(
            self.min.lon + (self.max.lon - self.min.lon) / 2, self.min.lat + (self.max.lat - self.min.lat) / 2
        )
...
class MercatorCoordinate:
    ...
    def to_tile(self, zoom):
        ...
        return TileCoordinate(zoom, transform(self.x), (2**zoom - 1) - transform(self.y))
    ...
    def to_geographic(self):
        return GeographicCoordinate(
            ...
            None
            if self.y is None
            else (2 * math.atan(math.exp(self.y * _180_originshift * _pi_180)) - _pi_2) * _180_pi,
        )
...
class RegionCoordinate(GridCoordinate):
    ...
    def geographic_bounds(self):
        ...
        p2 = TileCoordinate(
            self.root_tile.zoom, self.root_tile.x + self.tiles_per_row, self.root_tile.y + self.tiles_per_row
        ).to_geographic()
        return GeographicBB(p1.lon, p2.lat, p2.lon, p1.lat)
...
class GridBB:
    ...
    def is_inside(self, tile):
        return self.min.y <= tile.y <= self.max.y and (
            self.min.x <= tile.x <= self.max.x
            or (self.max.x > 2**tile.zoom - 1 and 0 <= tile.x <= self.max.x % (2**tile.zoom))
        )
