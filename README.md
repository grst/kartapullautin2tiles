...
Tiles are written as **lossless WebP** (`{z}/{x}/{y}.webp`) by default, which is about 25%
smaller than PNG and decodes slightly faster, at no cost in quality. Pass `--format png` if you
need to serve the tiles to a consumer that cannot read WebP.

`--webp-method` trades encoding time for file size and has no effect on decoding speed. The
default of 2 sits at the knee of that curve; raising it to 3 costs roughly 2.3x the encoding
time for another 3% off, and 6 is about 40x slower for 15%. Since `make-tiles` parallelizes
well (see above), a higher setting is affordable if you are generating tiles once and serving
them many times.
